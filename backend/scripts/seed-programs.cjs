...
const QUALITY = [
  ...
  },
  {
    ...
    order: 23,
    criteria: [
      "Действия сотрудника при получении информации устно/по телефону (записать в листок; прочитать вслух; услышать подтверждение; подшить в карту; врач подписывает в течение 24 ч)",
